...
constexpr char is_feeding_desc[] = "Feeding in Process";
...
attribute is_feeding_desc_attr = BuildStringAtt(DESCRIPTION_ATTR, const_cast<char *>(is_feeding_desc), sizeof(is_feeding_desc), ZCL_CHAR_STR);
...
attribute is_feeding_attr[]{
    {BINARY_PV_ATTR, const_cast<uint8_t *>(one_zero_byte), 1, ZCL_BOOL}, // present value
    binary_status_flag,
    out_of_service,
    is_feeding_desc_attr,
};
...
Cluster is_feeding_diag_clusters[] = {
    Cluster(BINARY_INPUT_CLUSTER_ID, is_feeding_attr, sizeof(is_feeding_attr) / sizeof(*is_feeding_attr))};
...
Endpoint ENDPOINTS[NUM_ENDPOINTS] = {
    ...
    Endpoint(4, ON_OFF_SENSOR, is_feeding_diag_clusters, out_clusters, sizeof(is_feeding_diag_clusters) / sizeof(*is_feeding_diag_clusters), 0)};